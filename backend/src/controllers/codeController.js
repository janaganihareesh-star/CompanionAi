...
exports.executeCode = async (req, res) => {
  ...
  try {
    ...
    exec(command, { timeout: 10000, cwd: tempDir }, async (error, stdout, stderr) => {
      const isDockerMissing = (error && error.message && error.message.includes('failed to connect to the docker API')) || 
                              (stderr && stderr.includes('failed to connect to the docker API')) ||
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      exec(command, { timeout: 15000, cwd: tempDir }, (error, stdout, stderr) => {
        const isDockerMissing = (error && error.message && error.message.includes('docker')) || (stderr && stderr.includes('docker'));
        if (isDockerMissing) {
